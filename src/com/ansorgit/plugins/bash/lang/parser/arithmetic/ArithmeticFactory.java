  ...
  implements BashTokenTypes, BashElementTypes
{
  ...
  private static final ArithmeticParsingFunction variableOperator = repeated(logicalOr, VARIABLE, ARITH_VARIABLE_OPERATOR_ELEMENT, "varOperator");
  private static final TernaryExpression ternary = new TernaryExpression(variableOperator);
  private static final ArithmeticParsingFunction simpleAssignment = new AbstractAssignment(ternary, TokenSet.create(new IElementType[] { EQ }));
  ...
  private static ArithmeticParsingFunction repeated(ArithmeticParsingFunction next, IElementType operator, IElementType marker, String debugInfo) {
    return repeated(next, TokenSet.create(new IElementType[] { operator }, ), marker, debugInfo);
  }
}
